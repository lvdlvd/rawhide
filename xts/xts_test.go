...
func TestXTS(t *testing.T) {
	for i, test := range xtsTestVectors {
		...
		if !bytes.Equal(ciphertext, expectedCiphertext) {
			t.Errorf("#%d: encrypt failed, got: %x, want: %x", i, ciphertext, expectedCiphertext)
			continue
		}
		...
		if !bytes.Equal(decrypted, plaintext) {
			t.Errorf("#%d: decrypt failed, got: %x, want: %x", i, decrypted, plaintext)
		}
	}
}
...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		{32, 512, false},  // AES-128-XTS
		{48, 512, false},  // AES-192-XTS
		{64, 512, false},  // AES-256-XTS
		{16, 512, true},   // Too short
		{128, 512, true},  // Too long
		{32, 15, true},    // Invalid sector size
		{32, 4096, false}, // Large sector size
	}

	for _, tt := range tests {
		key := make([]byte, tt.keyLen)
		_, err := New(key, tt.sectorSize)
		if (err != nil) != tt.wantErr {
			t.Errorf("New(key[%d], %d): got err=%v, wantErr=%v", tt.keyLen, tt.sectorSize, err, tt.wantErr)
		}
	}
}

func TestSectorEncryptDecrypt(t *testing.T) {
	key := make([]byte, 32)
	for i := range key {
		key[i] = byte(i)
	}
	...
	if err := c.EncryptSector(encrypted, 0); err != nil {
		t.Fatal(err)
	}
	...
	// Decrypt
	if err := c.DecryptSector(encrypted, 0); err != nil {
		t.Fatal(err)
	}
...
func TestDifferentSectorsDifferentCiphertext(t *testing.T) {
	...
	enc0 := make([]byte, 512)
	copy(enc0, plaintext)
	c.EncryptSector(enc0, 0)

	enc1 := make([]byte, 512)
	copy(enc1, plaintext)
	c.EncryptSector(enc1, 1)
...
// bytesBuffer for testing ReaderAt/WriterAt
...
func TestReaderAt(t *testing.T) {
	...
	// Encrypt it
	cipher, _ := New(key, 512)
	encrypted := make([]byte, 1024)
	copy(encrypted, plaintext)
	cipher.EncryptSectors(encrypted, 0)
